...
if ($method === 'GET') {
    ...
    $stmt = $pdo->prepare("SELECT a.id, a.title, a.cover, a.status, a.created_at, u.username AS author_name FROM articles a LEFT JOIN users u ON a.author_id = u.id WHERE a.status = 'published' ORDER BY a.created_at DESC LIMIT :per OFFSET :offset");
